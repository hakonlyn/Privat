...
public class UnitTestIssueResolver {
	...
	public void testIssueResolver() {
		...
		i = 1; s = allocateIssue(ch, Rank.STUDENT_ASSISTANT, i); Faculty handler =
		s.getHandler(); assertEquals(handler.getRank(), Rank.STUDENT_ASSISTANT);
		assertEquals(ch.getQueueSize(handler), 0);
		
		i = 2; s = allocateIssue(ch, Rank.STUDENT_ASSISTANT, i); handler =
		s.getHandler(); assertEquals(handler.getRank(), Rank.STUDENT_ASSISTANT);
		assertEquals(ch.getQueueSize(handler), 0);
		
		/* i = 3; s = allocateIssue(ch, Rank.HOD, i); handler = s.getHandler(); //
		Should return null because HOD queue is full. assertEquals(handler, null);
		assertEquals(ch.getQueueSize(handler), 1); */
		
		i = 4; s = allocateIssue(ch, Rank.PROFESSOR, i); handler = s.getHandler();
		assertEquals(handler.getRank(), Rank.PROFESSOR);
		assertEquals(ch.getQueueSize(handler), 0);
		
		i = 5; s = allocateIssue(ch, Rank.STUDENT_ASSISTANT, i); handler =
		s.getHandler(); assertEquals(handler.getRank(), Rank.STUDENT_ASSISTANT);
		assertEquals(ch.getQueueSize(handler), 0);
		
		// HOD is free now so check that the queue is empty.
		hodHandler1.issueCompleted(); assertEquals(ch.getQueueSize(hodHandler1), 0);
